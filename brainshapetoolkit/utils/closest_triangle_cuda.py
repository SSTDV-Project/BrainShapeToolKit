...
from . import indexing_torch as indexing
from .torch_bvh import construct_bvh
...
def closest_triangle(vf, points):
    """
    vf: f[*B, F, 3, 3]
    points: f[*B, P, 3]
    """
    ...
    *batch_size_p, num_points = points.shape[:-1]
    ...
    face_info[((batch_size*num_faces-1)//BLOCK + 1), (BLOCK)](
        sorted_vf.view(-1,3,3), face_inv, face_sym, face_obt
    )
    ...
    og_cfi = indexing.batch_indexing(sort2og, cfi)
